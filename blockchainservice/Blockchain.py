...
from .models import PatientBlockModel, PatientReport
...
class Blockchain:
    ...
    def __init__(self):
        ...
        for i in PatientBlockModel.objects.filter():
            j=Block(i.index, i.transactions, i.prev_hash)
            j.hash=i.hash
            self.chain.append(j)

    
    # Add a block to the chain after verfying and validating it. Input : block and hash of that block
    def add_block(self, block, hashl, report_id):
        prev_hash = self.last_block().hash
        #check the validity of the block
        if (prev_hash == block.prev_hash and self.is_valid(block, hashl)):
            block.hash = hashl
            PatientBlockModel.objects.create(index=block.index, hash=block.hash, prev_hash=block.prev_hash, nonce=block.nonce,
                                             transactions=PatientReport.objects.get(id=report_id))
    ...
    def mine(self):
        if(len(self.pending) > 0): #if there is atleast one pending transaction
            ...
            #add the block
            self.add_block(new_block, hashl, self.pending[0].get("report_id"))
